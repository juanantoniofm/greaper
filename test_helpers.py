...
class test_output(BaseTest):
    ...
    @nottest
    def test_sysout_capturing(self):
        """
        check if we can capture stderr
        TODO: we seem unable to capture and use the logging output from python
        """
        #self.C.begin()
        self.L.begin()
        output("a message","INFO")
        #self.C.finalize("what result")
        self.L.end()
        eq_("GREAP INFO a message", self.msg)
        eq_("GREAP INFO a message", "message")
